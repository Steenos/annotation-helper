...
- **Table Indexing** — Streamline annotation by auto-filling row, column, and span values for each cell. Includes an auto-fill button that appears next to the annotation toolbar.
  - Enter the number of columns in the table
  - toggle auto-fill button
  - select first table cell (top left) click autofill, then select the next cell to the right. Work left to right top to bottom.
## Installation
